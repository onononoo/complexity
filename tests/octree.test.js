...
test("octree: sphere leaves hug the surface", () => {
  const { g, root, info } = build_program("sphere(p, 1.0)");
  const b = localize_surface(g, root, info, 0, 4, 2);
  assert(b.surface_leaves > 0, "expected surface leaves");
...
test("octree: culls cells entirely outside and inside", () => {
  const { g, root, info } = build_program("sphere(p, 1.5)");
  const b = localize_surface(g, root, info, 0, 3, 3);
  assert(b.culled_outside > 0, "expected outside culling");
  assert(b.culled_inside > 0, "expected inside culling");
  assert(b.surface_leaves < b.total_leaves / 2, `too many leaves: ${b.surface_leaves}`);
});

test("octree: every true surface point lies in some leaf", () => {
  const { g, root, info } = build_program("torus(p, 1.0, 0.25)");
  const b = localize_surface(g, root, info, 0, 4, 2);
